...
Assert-Contains 'deploy\freqtrade.service' @('freqtradeorg/freqtrade:stable', '--memory=800m', '-p 127.0.0.1:8080:8080', 'ModelMacdMomentum')
...
Assert-Contains 'deploy\research\FiveModelStrategies.py' @('class ModelEmaAdxTrend', 'class ModelBollingerRsiReversion', 'class ModelDonchianAtrBreakout', 'class ModelMacdMomentum', 'class ModelSupertrendConsensus', 'return min(5.0, max_leverage)', 'timeframe = "15m"', 'stoploss = -0.03', '"0": 0.02')
...
Assert-Contains 'deploy\config.json.template' @('"dry_run": true', '"db_url": "sqlite:////freqtrade/user_data/tradesv3.dryrun.sqlite"', '"trading_mode": "futures"', '"margin_mode": "isolated"', '"max_open_trades": 3', '"stake_amount": 10', '"timeframe": "15m"', '"force_entry_enable": true', 'BTC/USDT:USDT', 'ETH/USDT:USDT', 'SOL/USDT:USDT', '__API_PASSWORD__')
...
Assert-Contains 'deploy\trade_learning.py' @('macd_momentum_long', 'macd_momentum_short', 'MACD 상향 교차', 'MACD 하향 교차')
